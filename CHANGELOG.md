...
- **2026-09-07** Skill / エージェントの評価（evals）の解説ページを新設（#138）
  ...
  - **SkillsBench の数値は arXiv アブストラクトを直接取得して確認** — 検索結果の要約に出た数値（86 タスク・11 ドメイン等）は一次情報と食い違ったため使わず、`arxiv.org/abs/2602.12670` 本文を直接取得して得た数値（87 タスク・8 ドメイン・18 model-harness 構成、平均成功率 33.9%→50.5%＝+16.6pt、構成単位の改善幅 +4.1〜+25.7pt）だけを採用した
