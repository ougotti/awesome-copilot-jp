...
| 日付 | 変更内容 |
|------|---------|
| 2026-09-15 | Kiro IDEのCVE-2026-89332を、承認画面の表示と副作用の停止を分けて検証する事例として追加。詳細はSkill / Pluginのセキュリティへ集約した |
...
> A2A エージェントの位置づけ（MCP との役割の違い、統治の集約）は [13 節](#エージェント間プロトコルの統治の集約--a2a-が-aaif-に合流) を参照してください。
...
**プロトコルレベルのセッションを廃止しても、アプリケーションが状態を持てなくなるわけではありません。** 呼び出しをまたいで状態を持たせたい場合は、tool から明示的なハンドルを発行し、モデルにそれを引数として渡し返させる設計になります。「stateless」は状態が失われることではなく、状態の持ち方が変わることを指します。

GitHub MCP Server は正式リリース前に先行対応済みです。ただし、この仕様変更が**誰にとって作業不要か**は、読者の役割によって異なります。

| 読者 | 確認すべきこと |
|------|---------------|
| **既製クライアントの利用者**（Copilot・Claude Code 等をそのまま使う） | 提供元の対応バージョン・更新案内を確認する。設定変更の要否を一律には断定しない |
| **SDK 利用の開発者**（MCP サーバー・クライアントを SDK で実装） | SDK の migration guide を確認する。**プロトコルの互換性と SDK API の互換性は別**。tier 1 SDK（TypeScript・Python・Go・C#）は対応済みだが、公式は「特に session identifier に依存していた実装には migration cost がある」と明記している |
| **独自サーバー・gateway の運用者** | session ID・ハンドシェイク・ヘッダー処理・状態保持への依存を点検する |
...
MCP のロードマップが**ツールと接続する側**の話であるのに対し、**エージェント同士が連携する側**にも動きがありました。2026-08-17、Google が開発した **A2A（Agent2Agent）** が、**Agentic AI Foundation（AAIF、Linux Foundation 傘下）**の hosted project として合流しました。AAIF は 2025-12-09 に MCP・goose・AGENTS.md を創設プロジェクトとして発足した組織で、これにより **MCP と A2A が同じ財団の下に並ぶ**ことになります。

**役割は競合ではなく、層が違います。** AAIF 自身が両者をこう整理しています。

| プロトコル | 担う層 |
|-----------|-------|
| MCP | **エージェントとツール**の接続 — ツール・データソース・アプリケーションへどう繋ぐか |
| A2A | **エージェントとエージェント**の連携 — 別フレームワーク・別ベンダーのエージェント同士が、相手の内部状態を知らずに発見・委譲・協働する |

7-2 節で扱った Agent Finder / ARD が「Catalog に A2A エージェントも含められる」としているのは、この位置づけと整合します（[7-2 節](#7-2-agent-finder--ard--必要な時に見つける)）。ARD が指す先のリソースの一部が、AAIF の下で統治される、という関係です。
