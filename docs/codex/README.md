...
| 使い分け | 向いている場面 |
|---------|--------------|
| `/plugins`（TUI） | 中身を見ながら選ぶ。有効・無効の切り替え |
| Plugin CLI | 手順をスクリプト化する。CI や環境構築で再現する |

> **CLI から入れる場合こそ、出自の確認を省かないでください。** UI の詳細画面を経由しない分、marketplace の source を明示的に確認する必要があります（[Skill / Plugin のセキュリティ](../dev-methods/skill-security.md#6-同名の別パッケージという入口)）。

### MCP tool の出力量を絞る（0.152.0）

0.152.0（2026-09-01）で、**MCP tool ごとに `output_token_limit` を設定**できるようになりました。セッションを resume した後も一貫して切り詰められます。

**MCP サーバー全体ではなく tool 単位の設定**である点が要点です。出力が大きい 1 つの tool だけを絞れるため、カタログ全体を諦めずにコンテキストを守れます。
