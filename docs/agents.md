...
> [github/awesome-copilot](https://github.com/github/awesome-copilot) で公開されている **184 個の Agents** を日本語で解説します。
...
Agents は **184 ファイル** あり、以下のカテゴリに分類されます：

| カテゴリ | ファイル数 | 主な用途 |
|---------|----------|---------|
| コード品質・レビュー | 約 18 | セキュリティ、アーキテクチャレビュー、ガバナンス |
| 計画・設計 | 約 22 | PRD、仕様書、実装計画、マルチエージェント調整 |
| メンタリング・教育 | 約 7 | 新人指導、学習支援、ドキュメント作成 |
| デバッグ・保守 | 約 6 | バグ調査、コードクリーンアップ |
| 言語・フレームワーク専門家 | 約 28 | .NET, React, Vue, Nuxt, Salesforce, WinUI 3 等 |
| MCP サーバー開発 | 約 10 | 各言語の MCP SDK |
| インフラ・DevOps | 約 18 | CI/CD, Kubernetes, Terraform, Terratest |
| Azure・クラウド | 約 12 | Azure アーキテクチャ、IaC、セキュリティ |
...
| テスト | 約 16 | TDD, Playwright, Polyglot テスト自動化 |
| AI・機械学習 | 約 5 | Python AI/ML, Notebook サンプル |
| 移行・モダナイゼーション | 約 3 | レガシー刷新、Oracle→PostgreSQL |
| 外部サービス連携 | 約 13 | 各種 SaaS 連携、科学論文検索 |
| 高度なモード | 約 8 | Beast モード、高性能推論 |
| その他 | 約 13 | 専門エージェント、a11y、LinkedIn |
