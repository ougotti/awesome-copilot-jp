...
> **対象ツール**: ツール横断（Claude Code・Codex・Cursor・GitHub Copilot CLI ほか） ｜ **実行環境**: CLI / IDE / デスクトップ（Codex App） ｜ **対象読者**: エンジニア ｜ **最終更新**: 2026-09-22
...
superpowers のスキルは、テスト・デバッグ・コラボレーション・メタの 4 カテゴリに分類されます。
...
>
> **2026-09 の変更**: デバッグカテゴリに **`diagnosing-superpowers`** が追加され、「superpowers を使ったセッションがうまくいかなかった理由」をトランスクリプトの証拠付きで報告できるようになりました。
...
#### diagnosing-superpowers — セッションの事後診断

| 項目 | 内容 |
|-----|------|
| **機能** | セッションのトランスクリプトを読み、何が起きたかを `path:line` の引用付きで報告 |
| **発動タイミング** | 「なぜ作業が重複したのか」「なぜ計画が無視されたのか」「なぜ時間・コストがかかったのか」を尋ねたとき |

superpowers を入れたセッションが期待どおりに動かなかったとき、その原因を**推測ではなく記録から**突き止めるためのスキルです。現在のセッションだけでなく、ID やパスで指定した過去のセッションも対象にできます。

**進め方：**

| ステップ | 内容 |
|--------|------|
| **1. 問題の特定** | 対象セッション・期待した挙動・実際の挙動・着目する指標を、1 問ずつの質問で言語化する |
| **2. 特定** | セッションファイルの絶対パスを確定し、サブエージェントのトランスクリプトも列挙する |
| **3. 分析** | スキル発動履歴・計画遵守・作業の重複・つまずき・品質・要求の衝突・コストと時間の 7 観点を、サブエージェントで並列に調査する |
| **4. 報告** | 調査結果をレポートにまとめ、ファイルパスとともに提示する |
| **5〜7.（任意）** | 既存 Issue の検索と起票、機微情報を除去した bug report バンドルの作成、類似セッションの洗い出し |

**Iron Law：**
- すべての指摘に `path:line` の引用を付ける。引用がなければ指摘として採用しない
- 数値は必ずトランスクリプトか実行したコマンドから取る。記憶に頼らない
- セッションファイルを変更・移動・削除しない（読み取り専用）
- **superpowers 側の不具合を断定しない** — 事実と証拠の報告までを担い、修正の要否は Issue をトリアージする側が判断する
- バンドルのアーカイブ化と Issue 起票は、内容を利用者が確認して承認してから行う

> **注意**: バンドルにはセッションの記録が含まれます。機微情報の除去（scrub）は行われますが、upstream のスキル自身も「見落としはあり得るので共有前に全ファイルを確認すること」と明記しています。共有前に中身を自分で確認してください。
