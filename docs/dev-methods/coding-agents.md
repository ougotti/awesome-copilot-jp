...
> **対象ツール**: ツール横断（Claude Code・Codex・Qwen Code・OpenCode・Bionic・Jules・Antigravity） ｜ **実行環境**: CLI / デスクトップ / IDE / Cloud ｜ **対象読者**: エンジニア ｜ **最終更新**: 2026-09-23
...
| マージ後の維持コスト | 同種・同規模のPRでrevert、手直し、security finding、レビュー時間を測れるか |
...
公開PRの観察研究は、マージ後90日の指標を考える材料になりますが、タスク割当やPRサイズが揃わない製品ランキングには使えません。**→ 指標の定義と研究の限界は [Skill / エージェントの評価](evals.md#コーディングエージェントはマージ後も測る) を参照。**
...
ここで挙げたツールが備えるサブエージェント機能を**いつ使うべきか**は別の判断です。**→ [マルチエージェントを使う境界線](multi-agent.md)**
