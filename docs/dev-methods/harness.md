...
| 部品 | 内容 |
|------|------|
| 永続セッション | 並行する独立したセッションを持ち、Gateway の再起動後も再開できる。過去のセッションを検索して文脈を引き継ぐ |
...
- 更新チャネルは Stable（既定）・Insider・Nightly。Insider は Stable と同じアプリの別レーン、Nightly は別アプリとして並存する。公式の記述に Preview / Beta の表記はなく、本ガイドでは `状態`: GA として扱う
