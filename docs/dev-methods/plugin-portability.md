...
いずれもクラウドベンダー公式のプラグイン集で、複数のエージェント向けに配っていますが、可搬性の結果は割れます（確認日: 2026-09-18）。

| | [aws/agent-toolkit-for-aws](https://github.com/aws/agent-toolkit-for-aws) の `aws-core` | [google/skills](https://github.com/google/skills/tree/main/plugins/cloud/google-cloud-developer) の `google-cloud-developer` | [microsoft/azure-skills](https://github.com/microsoft/azure-skills) の `azure` | [microsoft/power-platform-skills](https://github.com/microsoft/power-platform-skills) の `power-pages` |
|---|---|---|---|---|
| 提供元 | `Official`（Amazon Web Services） | `Official`（Google） | `Official`（Microsoft） | `Official`（Microsoft） |
| 状態 | `GA` | `—`（公式発表に GA / Preview の明記なし） | プラグインごとに確認 | プラグインごとに確認 |
| `plugin.json` の場所 | プラグインルート直下 | プラグインルート直下 | `.claude-plugin/` の下のみ | `.claude-plugin/` の下のみ |
| `$schema` | **あり**（1.0.0 の正式な識別子） | **あり**（1.0.0 の正式な識別子） | **なし** | **なし** |
| 最上位フィールド | 10 個の範囲に収まっている | 10 個の範囲に収まっている | `skills` / `mcpServers` / `hooks` で場所を指定 | 10 個の範囲だが `$schema` を欠く |
| MCP 設定 | `mcp.json`（ルート） | `mcp.json`（ルート、Developer Knowledge MCP） | `.mcp.json`（`mcpServers` で指定） | `.mcp.json` |
| クライアント固有の対応 | `extensions` と `.claude-plugin/` / `.codex-plugin/` / `.cursor-plugin/` | `.claude-plugin/` / `.codex-plugin/` 等を標準マニフェストと併置 | `hooks` フィールドで `hooks/claude-hooks.json` を指す | `hooks/` にそのまま |
| 判定 | **可搬形式** | **可搬形式** | 可搬形式ではない | 可搬形式ではない |
...
`google-cloud-developer` も、ルートの `plugin.json` と `mcp.json` を Agent Plugins 1.0.0 に合わせた可搬形式です。Skillsに加えて、公式ドキュメントを検索する **Developer Knowledge MCP** の設定を束ねています。一方、`.codex-plugin/plugin.json` と `.claude-plugin/plugin.json` は各クライアント向けのメタデータです。これらを併置していても、可搬性を判定する入口はあくまで**ルートの標準マニフェスト**です。
...
> [google/skills](https://github.com/google/skills) には、従来どおり `skills/` 配下で `SKILL.md` を直接配る入口と、`plugins/` 配下で SkillsとMCPを束ねる入口の**両方**があります。前者のSkill単体にはこの判定表を使いません。後者は各プラグインのルート `plugin.json` を個別に確認します。
