...
### 学習支援・応答の質（Learning & Response Quality）

このカテゴリの 2 つは、**成果物を作るのではなく、Claude の応答そのものに一手間を加える**タイプのスキルです。ユーザーが明示的に呼び出すのではなく、条件に合致したときに Claude 側が自動で発動します。

#### academy-guide — 学習リソースの案内

| 項目 | 内容 |
|-----|------|
| **機能** | 回答に加えて [Claude Academy](https://academy.claude.com) の該当コース・チュートリアルを案内 |
| **使用場面** | Claude 製品の使い方を学びたいとき |
| **対象** | 学習中のユーザー（作業の実行中は発動しない） |

**発動する質問の例：** 「どうやって〜する？」「〜を始めるには」「Claude に何ができる？」、Artifacts・Projects・Skills・Plugins・Connectors・MCP に関する質問、チームや組織への展開の相談、研修・オンボーディング資料の依頼。

**設計上の要点：**
- **まず質問に答え**、推奨は補足として最大 2 件まで添える
- 案内する URL は**取得したカタログの実データのみ**を使い、存在しないコースを作らない
- 一致が弱いときは**推奨しない**（誤った推奨 1 件の損失が、正しい推奨 10 件の価値を上回るという判断）
- 「作業を進めたいだけ」のユーザーには発動しない

#### discernment-nudge — 鵜呑みにしないための問い返し

| 項目 | 内容 |
|-----|------|
| **機能** | 実行に移せる回答の後に、確認すべき点を突く短い質問を 2〜3 個添える |
| **使用場面** | 見積もり・助言・分析・提案文など、そのまま行動につながる出力 |
| **対象** | 1 つの会話につき **1 回だけ**発動 |

**対象になる出力：** 数値の見積もり・予測、経営／健康／法務／財務／キャリア／対人といった影響の大きい助言、事実・歴史に関する主張、多段階の推論や分析、データの解釈、目標・計画・提案・メールなどの下書き。

**発動しない場合：** 単純な調べもの、純粋に教育的な説明、提供された内容の整形・変換のみ、実行するコードの作成、創作や雑談、そしてユーザーが既に「裏取りして」「出典を出して」と依頼済みの場合。

**出力形式：** 「A few things worth a second look:」の一行に続けて、**回答中の具体的な数値・前提・抜けている文脈**に紐づく質問をプレーンテキストの箇条書きで示します。一般論ではなく、その回答の中身を指す質問である点が要件です。

> この 2 つは、Skill が「作業の代行」だけでなく、**応答の作法そのもの**を定義する用途にも使えることを示す例になっています。
...
| **学習支援・応答の質** | academy-guide, discernment-nudge | 学習リソースの案内、回答の検証を促す問い返し |
